...
/**
 * バッチ更新最適化機能のサンプル(sortメソッド＆拡張ソートクラス使用)<br>
 * <p>
 * 事前準備：PERSONAL_INFORMATIONテーブルが存在し、101～300までのデータが存在する事<br>
 * </p>
 * <p>
 * サンプル内容：insert99,delete09,update01,insert99...の順に格納したSQLが<br>
 * バッチ更新実行時にupdate100件,delete100件,insert100件の順に<br>
 * 最適化されていることを確認するサンプル<br>
 * </p>
 */
@Component
public class B006002BLogic implements BLogic {

    private Log log = LogFactory.getLog(B006002BLogic.class);
    ...
    public int execute(BLogicParam param) {
        ...
        Collector<SampleFileLineObject> collector = new FileCollector<SampleFileLineObject>(
                csvFileQueryDAO, "C:\\tmp\\inputB006.csv", SampleFileLineObject.class);
